...
class ProcessWiki:
    ...
    def __init__(self, paragraphs, remove_stop_word=True, use_stemmer=True):
    ...
    def computeTFIDF(self):
        ...
        for index in range(0, len(self.paragraphs)):
            word_frequency = self.getTermFrequencyCount(self.paragraphs[index])
            self.paragraphInfo[index] = {}
            self.paragraphInfo[index]['wF'] = word_frequency

        word_paragraph_frequency = {}
        for index in range(0, len(self.paragraphInfo)):
            for word in self.paragraphInfo[index]['wF'].keys():
                if word in word_paragraph_frequency.keys():
                    word_paragraph_frequency[word] += 1
                else:
                    word_paragraph_frequency[word] = 1

        self.idf = {}
        for word in word_paragraph_frequency:
            # Adding Laplace smoothing by adding 1 to total number of documents
            self.idf[word] = math.log((self.totalParas + 1) / word_paragraph_frequency[word])
    ...
    def query(self, pQ):
        ...
        sentences = relevantSentences[0][0]
        ps = PorterStemmer()
